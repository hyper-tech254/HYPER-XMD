const { hxmd } = require("../hyper"),
        ...
        { generateWAMessageContent, generateWAMessageFromContent } = require('hyper-baileys');
...
hxmd({
  ...
  async (from, Hyper, conText) => {
    ...
    await Hyper.relayMessage(from, message.message, { messageId: message.key.id });
...
hxmd({
  ...
  async (from, Hyper, conText) => {
    ...
    try {
      ...
      tempFilePath = await Hyper.downloadAndSaveMediaMessage(quotedAudio || quotedVideo, 'temp_media');
