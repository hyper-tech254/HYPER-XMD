...
hxmd({
  ...
  if (!isSuperUser) {
    ...
    try {
        ...
        const { data: commitData } = await axios.get("https://api.github.com/repos/hyper-tech254/hyper-xmd/commits/main");
        const latestCommitHash = commitData.sha;

        const currentHash = await getCommitHash();

        if (latestCommitHash === currentHash) {
            return reply("✅ Your Hyper-xmd Bot is Already on the Latest Version!");
        }
        ...
        const { data: zipData } = await axios.get("https://github.com/hyper/hyper-xmd/archive/main.zip", { responseType: "arraybuffer" });
