...
hxmd({ 
  ...
  try {
    await Hyper.groupSettingUpdate(from, 'not_announcement');
    const userNumber = sender.split('@')[0];
...
hxmd({ 
  ...
  try {
    await Hyper.groupSettingUpdate(from, 'announcement');
    const userNumber = sender.split('@')[0];
...
gmd({ 
  ...
  try {
    const gInfo = await Hyper.groupMetadata(from);
    console.log(gInfo);
...
• Community: ${gInfo.isCommunity ? '✅' : '❌'}
    ...
    await react("❌");
    await Hyper.sendMessage(from, { text: "Failed to fetch group metadata." }, { quoted: mek });
  }
});
...
hxmd({
  ...
  try {
    await Hyper.groupParticipantsUpdate(from, [finalResult], 'demote'); 
...
gmd({
  ...
  try {
    await Hyper.groupParticipantsUpdate(from, [finalResult], 'promote'); 
