const { hxmd, commands } = require("../hyper");
const fs = require('fs').promises;
const fsA = require('node:fs');
const { S_WHATSAPP_NET } = require("hyper-baileys");
...
hxmd({
  ...
  const { 
    ...
    uploadToHyperCdn, uploadToPasteboard, uploadToCatbox, 
    newsletterUrl, newsletterJid, HyperTechApi, HyperApiKey, 
    botPrefix, hxmdBuffer, hxmdJson, formatAudio, formatVideo, timeZone 
  } = conText;
...
gmd({
  ...
  try {
    ...
    let finalResult = result;
    if (result && result.includes('@lid')) {
      finalResult = await Hyper.getJidFromLid(result);
    }
    const userNumber = finalResult.split("@")[0];
...
gmd({
  ...
  try {
    ...
    if (target.startsWith('@') && !target.includes('@lid')) {
      ...
      try {
        const lid = await Hyper.getLidFromJid(target);
        if (lid) {
          target = lid;
...
hxmd({
  ...
  try {
    ...
    tempFilePath = await Hyper.downloadAndSaveMediaMessage(quotedImg, 'temp_media');
  ...
  try {
    if (quoted) {
      if (isGroup && !targetUser.endsWith('@s.whatsapp.net')) {
        try {
          const jid = await Hyper.getJidFromLid(targetUser);
      ...
      try {
        profilePictureUrl = await Hyper.profilePictureUrl(targetUser, "image");
      } catch (error) {
      ...
      try {
        const statusData = await Hyper.fetchStatus(targetUser);
...
hxmd({
  ...
  try {
    ...
    const tempFilePath = await Hyper.downloadAndSaveMediaMessage(quotedImg, 'temp_media');
    const imageBuffer = await fs.readFile(tempFilePath);
    try {
      await Hyper.updateProfilePicture(Hyper.user.id, { url: tempFilePath });
...
hxmd({
  ...
  try {
    if (quoted) {
      try {
        profilePictureUrl = await Hyper.profilePictureUrl(quotedUser, "image");
...
hxmd({
  ...
  try {
    let profilePictureUrl;
    try {
      profilePictureUrl = await Hyper.profilePictureUrl(from, "image");
    } catch (error) {
      await react("❌");
...
hxmd({ 
  ...
  description: "Reveal View Once Media"
    ...
    try {
        ...
        await Hyper.sendMessage(from, msg);
      await react("✅");
    } catch (e) {
...
hxmd({ 
  ...
  description: "Reveal View Once Media"
    ...
    try {
        ...
        await Hyper.sendMessage(Hyper.user.id, msg);
      await react("✅");
    } catch (e) {
        console.error("Error in vv command:", e);
        reply(`Error: ${e.message}`);
