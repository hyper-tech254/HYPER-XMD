const { hxmd, toAudio, toVideo, toPtt, stickerToImage, hxmdFancy, hxmdRandom } = require("../hyper");
...
async function hxmdSticker(file, options) {
    let stickerBuffer;
    let attempts = 0;
    ...
    try {
        ...
        try {
            if (quotedImg || quotedVideo) {
                ...
                if (quotedVideo) {
                    const compressedFile = hxmdRandom(".webp");
                ...
                const stickerBuffer = await hxmdSticker(mediaFile, {
                ...
                await fs.unlink(mediaFile).catch(() => {});
                await react("✅");
                return Hyper.sendMessage(from, { sticker: stickerBuffer }, { quoted: mek });
                ...
                const stickerFile = hxmdRandom(".webp");
                await fs.writeFile(stickerFile, stickerData);

                const newStickerBuffer = await hxmdSticker(stickerFile, {
                    pack: config.PACK_NAME, 
                    author: config.PACK_AUTHOR,
                    type: q.includes("--crop") || q.includes("-c") ? StickerTypes.CROPPED : StickerTypes.FULL,
                    categories: ["🤩", "🎉"],
                ...
                await react("✅");
                return Hyper.sendMessage(from, { sticker: newStickerBuffer }, { quoted: mek });
            }
        } finally {
...
gmd({
    ...
    try {
        ...
        let tempFilePath;
        try {
            tempFilePath = await Hyper.downloadAndSaveMediaMessage(quotedSticker, 'temp_media');
...
gmd({
  ...
  async (from, Hyper, conText) => {
    ...
    let tempFilePath;
    try {
      tempFilePath = await Hyper.downloadAndSaveMediaMessage(quotedVideo, 'temp_media');
...
hxmd({
  ...
  async (from, Hyper, conText) => {
    ...
    let tempFilePath;
    try {
      tempFilePath = await Hyper.downloadAndSaveMediaMessage(quotedAudio, 'temp_media');
      const buffer = await fs.readFile(tempFilePath);
