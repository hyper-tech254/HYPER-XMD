...
hxmd({
  ...
  async (from, Hyper, conText) => {
    ...
    try {
      const buffer = await hxmdBuffer(q);
...
hxmd({
  ...
  async (from, Hyper, conText) => {
    const { q, mek, reply, react, sender, botFooter, hxmdBuffer, formatAudio } = conText;
...
hxmd({
  ...
  async (from, Hyper, conText) => {
    const { q, mek, reply, react, sender, botPic, botName, botFooter, newsletterUrl, newsletterJid, hxmdJson, hxmdBuffer, formatAudio, HyperTechApi, HyperApiKey } = conText;
...
/* hxmd({
  ...
  async (from, Hyper, conText) => {
    ...
    try {
      ...
      const response = await hxmdBuffer(audioApi);
...
╰────────────────◆`,
      ...
      const messageSent = await Hyper.sendMessage(from, infoMess, { quoted: mek });
  ...
  async (from, Hyper, conText) => {
    ...
    try {
      ...
      const videoUrl = firstVideo.url;
        ...
        const audioApi = `${HyperTechApi}/api/download/ytmp4?stream=true&apikey=${HyperApiKey}&url=${encodeURIComponent(videoUrl)}`;
