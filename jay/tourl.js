const { hxmd } = require("../hyper");
const path = require("path");
const fs = require('fs').promises;
const { sendButtons } = require('hyper-btns');
const { downloadContentFromMessage, generateWAMessageFromContent, normalizeMessageContent } = require('hyper-baileys');

hxmd({
    pattern: "giftedcdn",
    react: "⬆️",
    category: "uploader",
    description: "Upload any file to HyperCDN",
}, async (from, Hyper, conText) => {
    await handleUpload(from, Hyper, conText, 'hypercdn');
});
...
hxmd({
    ...
    await handleUpload(from, Hyper, conText, 'githubcdn');
});
...
hxmd({
    ...
    await handleUpload(from, Hyper, conText, 'catbox');
});
...
hxmd({
    ...
    await handleUpload(from, Hyper, conText, 'pixhost');
});
...
hxmd({
    ...
    await handleUpload(from, Hyper, conText, 'pasteboard');
});

async function handleUpload(from, Hyper, conText, service) {
    const { mek, reply, react, botFooter, botPrefix, quoted, getMediaBuffer, uploadToHyperCdn, uploadToGithubCdn, uploadToPixhost, getFileContentType, uploadToImgBB, uploadToPasteboard, uploadToCatbox, pushName, newsletterUrl } = conText;
    ...
    try {
        ...
        if (quotedImg) {
            ...
            if (service !== 'catbox' && service !== 'hypercdn' && service !== 'githubcdn') {
                return reply(`❌ ${service} only supports images. Use ${botPrefix}catbox or ${botPrefix}hypercdn or ${botPrefix}githubcdn  for videos and any other file type.`);
            }
            ...
            if (service !== 'catbox' && service !== 'hypercdn' && service !== 'githubcdn') {
                return reply(`❌ ${service} only supports images. Use ${botPrefix}catbox or ${botPrefix}hypercdn or ${botPrefix}githubcdn  for audios and any other file type.`);
            }
            ...
            if (service === 'pixhost') {
                return reply(`❌ ${service} does not support sticker uploads. Use ${botPrefix}imgbb, ${botPrefix}pasteboard, ${botPrefix}catbox or ${botPrefix}hypercdn or ${botPrefix}githubcdn  instead.`);
            }
            ...
            if (service !== 'catbox' && service !== 'hypercdn' && service !== 'githubcdn') {
                return reply(`❌ ${service} only supports images. Use ${botPrefix}catbox or ${botPrefix}hypercdn or ${botPrefix}githubcdn  for documents and any other file type.`);
            }
        ...
        if (!isImage && service !== 'catbox' && service !== 'hypercdn' && service !== 'githubcdn') {
            return reply(`❌ ${service} only supports image files. Use ${botPrefix}catbox or ${botPrefix}hypercdn or ${botPrefix}githubcdn for any other file types.`);
        }
