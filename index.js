...
app.get("/", (req, res) => res.sendFile(__dirname + "/hyper/hyper-xmd.html"));
app.listen(PORT, () => console.log(`Server Running on Port: ${PORT}`));

const sessionDir = path.join(__dirname, "hyper", "session");
...
async function startGifted() {
    try {
        ...
        Hyper = hyperConnect(hyperSock);
        
        store.bind(Hyper.ev);

        Hyper.ev.process(async (events) => {
        ...
        if (autoReact === "true") {
            Hyper.ev.on('messages.upsert', async (mek) => {
                ms = mek.messages[0];
                try {
                    if (ms.key.fromMe) return;
                    if (!ms.key.fromMe && ms.message) {
                        const randomEmoji = emojis[Math.floor(Math.random() * emojis.length)];
                        await HyperAutoReact(randomEmoji, ms, Hyper);
                    }
                } catch (err) {
        ...
        let giftech = { chats: {} };
const botJid = `${Hyper.user?.id.split(':')[0]}@s.whatsapp.net`;
const botOwnerJid = `${Hyper.user?.id.split(':')[0]}@s.whatsapp.net`;
...
Gifted.ev.on("messages.upsert", async ({ messages }) => {
    try {
        ...
        if (!hypertech.chats[key.remoteJid]) hypertech.chats[key.remoteJid] = [];
        hypertech.chats[key.remoteJid].push({
            ...ms,
        ...
        if (hypertech.chats[key.remoteJid].length > 50) {
            hypertech.chats[key.remoteJid] = hypertech.chats[key.remoteJid].slice(-50);
        }
        ...
        if (ms.message?.protocolMessage?.type === 0) {
            ...
            hypertech.chats[key.remoteJid] = hypertech.chats[key.remoteJid].filter(m => m.key.id !== deletedId);
        }
    } catch (error) {
        ...
        if (autoBio === 'true') {
            setTimeout(() => HyperAutoBio(Hyper), 1000);
            setInterval(() => HyperAutoBio(Hyper), 1000 * 60); // Update every minute 
        }

        Hyper.ev.on("call", async (json) => {
            await HyperAnticall(json, Hyper);
        });

        Hyper.ev.on("messages.upsert", async ({ messages }) => {
            if (messages && messages.length > 0) {
                await HyperPresence(Hyper, messages[0].key.remoteJid);
            }
        });

        Hyper.ev.on("connection.update", ({ connection }) => {
            if (connection === "open") {
                logger.info("Connection established - updating presence");
                HyperPresence(Hyper, "status@broadcast");
            }
        });

        if (chatBot === 'true' || chatBot === 'audio') {
            HyperChatBot(Hyper, chatBot, chatBotMode, createContext, createContext2, googleTTS);
        }
        ...
        Hyper.ev.on('messages.upsert', async ({ messages }) => {
            ...
            if (antiLink !== 'false') {
                await HyperAntiLink(Hyper, message, antiLink);
            }
        });
      ...
      try {
        ...
        if (mek.key && mek.key?.remoteJid === "status@broadcast" && isJidBroadcast(mek.key.remoteJid)) {
            const hypertech = jidNormalizedUser(Hyper.user.id);

            if (autoReadStatus === "true") {
                await Hyper.readMessages([mek.key, hypertech]);
            }
            ...
            if (autoLikeStatus === "true" && mek.key.participant) {
                ...
                await Hyper.sendMessage(
                    ...
                    { statusJidList: [mek.key.participant, hypertech] }
                );
            }

            if (autoReplyStatus === "true") {
                if (mek.key.fromMe) return;
                const customMessage = statusReplyText || '✅ Status Viewed By Hyper-xmd';
                await Hyper.sendMessage(
                    fromJid,
                    { text: customMessage },
                    { quoted: mek }
        ...
        console.error("Error Processing Actions:", error);
         ...
         try {
            const pluginsPath = path.join(__dirname, "hyper");
        ...
        Hyper.ev.on("messages.upsert", async ({ messages }) => {
            ...
            const botId = standardizeJid(Hyper.user?.id);
...
const sendr = ms.key.fromMe 
                ? (Hyper.user.id.split(':')[0] + '@s.whatsapp.net' || Hyper.user.id) 
...
if (groupInfo && groupInfo.participants) {
    ...
    isSuperAdmin = groupSuperAdmins.includes(sender);
            ...
            const pushName = ms.pushName || 'Hyper-xmd User';
            const quoted = 
            ...
            const user = mentionedJid.length > 0 
                ...
                : repliedMessage 
                    ...
                    : '';
const devNumbers = ('254794597254,254113873107,254702431142,254786827982,')
    .split(',')
...
if (autoBlock && sender && !isSuperUser && !isGroup) {
    ...
    if (countryCodes.some(code => sender.startsWith(code))) {
        try {
            await Hyper.updateBlockStatus(sender, 'block');
        } catch (blockErr) {
            console.error("Block error:", blockErr);
            if (isSuperUser) {
                await Hyper.sendMessage(ownerJid, { 
            ...
            if (autoRead === "true") await Hyper.readMessages([ms.key]);
            ...
            if (isCommandMessage && cmd) {
                ...
                if (gmd) {
                    ...
                    try {
                        const reply = (teks) => {
  Hyper.sendMessage(from, { text: teks }, { quoted: ms });
};
                            ...
                            try {
                                await Hyper.sendMessage(from, { 
                                    ...
                                    ...createContext(sender, {
                                        title: options.title || groupName || botName || "HYPER-XMD",
                                        body: options.body || ""
                                    })
                                }, { quoted: ms });
                            } catch (err) {
                        ...
                        Hyper.getJidFromLid = async (lid) => {
    const groupMetadata = await Hyper.groupMetadata(from);
    const match = groupMetadata.participants.find(p => p.lid === lid || p.id === lid);
    return match?.pn || match?.phoneNumber || null;
};

Hyper.getLidFromJid = async (jid) => {
    ...
    return match?.lid || null;
                        ...
                        await gmd.function(from, Hyper, conText);
...
> *${botCaption}*`;
            ...
            if (connection === "close") {
                ...
                if (reason === DisconnectReason.badSession) {
                    ...
                    try {
                        await fs.remove(__dirname + "/hyper/session");
                    } catch (e) {
