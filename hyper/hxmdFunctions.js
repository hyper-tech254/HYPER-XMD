...
async function withTempFiles(inputBuffer, extension, processFn) {
  const tempInput = `hyper/temp/temp_${Date.now()}.input`;
  const tempOutput = `hyper/temp/temp_${Date.now()}.${extension}`;
...
async function formatAudio(buffer) {
  const inputPath = `hyper/temp/temp_in${Date.now()}.mp3`;
  const outputPath = `hyper/temp/temp_out${Date.now()}.mp3`;
...
async function formatVideo(buffer) {
  const inputPath = `hyper/temp/temp_in${Date.now()}.mp4`;
  const outputPath = `hyper/temp/temp_out${Date.now()}.mp4`;
...
async function loadSession() {
    try {
        ...
        if (header !== "Hyper" || !b64data) {
            throw new Error("❌ Invalid session format. Expected 'Hyper-xmd~.....'");
        }
...
const gmdBuffer = async (url, options = {}) => {
    try {
        ...
        console.error("hxmdBuffer Error:", err);
        return err;
    }
};
...
const gmdJson = async (url, options = {}) => {
    try {
        ...
        console.error("hxmdJson Error:", err);
        return err;
    }
};
...
function verifyJidState(jid) {
    ...
    console.log('Welcome to Hyper xmd', jid);
    return true;
}
