...
const { createContext } = require("./hxmdHelpers");
const logger = require("hyper-baileys/lib/Utils/logger").default.child({});
const { isJidGroup, downloadMediaMessage } = require("hyper-baileys");
...
const HyperApiKey = 'hyper';
const HyperTechApi = 'https://api.giftedtech.co.ke';
async function HyperAutoReact(emoji, ms,Gifted) {
  try {
    const react = {
      react: {
        text: emoji,
    ...
    await Hyper.sendMessage(ms.key.remoteJid, react);
  } catch (error) {
...
const HyperAntiLink = async (Hyper, message, antiLink) => {
    try {
        ...
        await Hyper.sendMessage(from, { delete: message.key });

        if (antiLink === 'kick') {
            await Hyper.groupParticipantsUpdate(from, [sender], 'remove');
            await Hyper.sendMessage(
                from,
                {
                    text: `⚠️ ${botName || 'Hyper xmd'} anti-link active!\nUser @${sender.split('@')[0]} has been kicked for sharing a link.`,
                    mentions: [sender],
                }
            );
...
const HyperAutoBio = async (Gifted) => {
                try {
                    ...
                    await Hyper.updateProfileStatus(bioText);
                } catch (error) {
                }
            };
...
const availableApis = [
    `${HyperTechApi}/api/ai/ai?apikey=${HyperApiKey}&q=`,
    `${HyperTechApi}/api/ai/mistral?apikey=${HyperApiKey}&q=`,
    `${HyperTechApi}/api/ai/meta-llama?apikey=${HyperApiKey}&q=`
];
...
async function getAIResponse(query) {
    if (isIdentityQuestion(query)) {
        return 'I am an Interactive Ai Assistant Chat Bot, created by Hyper Tech!';
    }
...
function HyperChatBot(Gifted, chatBot, chatBotMode, createContext, createContext2, googleTTS) {
    if (chatBot === 'true' || chatBot === 'audio') {
        Hyper.ev.on("messages.upsert", async ({ messages }) => {
            try {
                ...
                if (chatBot === "true") {
                    await Hyper.sendMessage(jid, { 
                        text: String(aiResponse),
                        ...createContext(jid, {
                            title: `${botName} 𝐂𝐇𝐀𝐓 𝐁𝐎𝐓`,
                            body: '𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐛𝐲 𝐇𝐲𝐩𝐞𝐫 𝐀𝐩𝐢'
                        })
                    }, { quoted: msg });
                }
...
const HyperPresence = async (Hyper, jid) => {
    try {
        const isGroup = jid.endsWith('@g.us');
        ...
        await Hyper.sendPresenceUpdate(whatsappPresence, jid);
...
const HyperAnticall = async (json, Hyper) => {
   for (const id of json) {
      if (id.status === 'offer') {
         if (antiCall === "true" || antiCall === "decline") {
            let msg = await Hyper.sendMessage(id.from, {
               text: `${antiCallMsg}`,
               mentions: [id.from],
            });
            await Hyper.rejectCall(id.id, id.from);
            ...
            await Hyper.rejectCall(id.id, id.from); 
            await Hyper.updateBlockStatus(id.from, "block");
         }
      }
   }
};
...
const HyperAntiDelete = async (Hyper, deletedMsg, key, deleter, sender, botOwnerJid, deleterPushName, senderPushName) => {
            ...
            promises.push((async () => {
                try {
                    ...
                    if (deletedMsg.message?.conversation || deletedMsg.message?.extendedTextMessage?.text) {
                        ...
                        await Hyper.sendMessage(key.remoteJid, {
                            text: `${baseAlert}\n\n📝 *Content:* ${text}`,
                            mentions: [deleter],
                            ...context
                        });
            ...
            promises.push((async () => {
                try {
                    ...
                    if (deletedMsg.message?.conversation || deletedMsg.message?.extendedTextMessage?.text) {
                        ...
                        await Hyper.sendMessage(botOwnerJid, { 
...
module.exports = { logger, emojis, HyperAutoReact, HyperTechApi, HyperApiKey, HyperAntiLink, HypeAutoBio, HyperChatBot, HyperAntiDelete, HyperAnticall, HyperPresence };
